...
impl<'tx> PathfinderStateReader<'tx> {
    ...
    // TODO Try to remove the class_hash param, maybe return an option?
    fn non_cached_compiled_contract_class(&mut self, pathfinder_class_hash: ClassHash, class_hash: &starknet_api::core::ClassHash) -> Result<(Option<BlockNumber>, blockifier::execution::contract_class::ContractClass), StateError> {
            ...
            casm_definition.map_err(map_anyhow_to_state_err)?
        {
            ...
            return Ok((definition_block_number, blockifier::execution::contract_class::ContractClass::V1(
                casm_class,
            )));
        }
        ...
        let definition = if self.ignore_block_number_for_classes {
            ...
            self.transaction
                ...
                // TODO That's ugly
                .map(|option| option.map(|(block_number, definition)| (Some(block_number), definition)))
        };

        if let Some((definition_block_number, definition)) = definition.map_err(map_anyhow_to_state_err)? {
            ...
            return Ok((definition_block_number, blockifier::execution::contract_class::ContractClass::V0(
                class,
            )));
        }
...
impl StateReader for PathfinderStateReader<'_> {
    ...
    > {
        ...
        let (definition_block_number, contract_class) = self.non_cached_compiled_contract_class(pathfinder_class_hash, class_hash)?;
